...
def dump_results(end_points, dump_dir, config,inference_switch=False):
    ...
    pred_heading_residual = torch.gather(end_points['last_heading_residuals'], 2, pred_heading_class.unsqueeze(-1)) # B,num_proposal,1
    pred_heading_class = pred_heading_class.detach().cpu().numpy() # B,num_proposal
    pred_heading_residual = pred_heading_residual.squeeze(2).detach().cpu().numpy() # B,num_proposal
    pred_size_class = torch.argmax(end_points['last_size_scores'], -1) # B,num_proposal
    pred_size_residual = torch.gather(end_points['last_size_residuals'], 2, pred_size_class.unsqueeze(-1).unsqueeze(-1).repeat(1,1,1,3)) # B,num_proposal,1,3
    ...
    for i in range(batch_size):
        ...
        if np.sum(objectness_prob>DUMP_CONF_THRESH)>0:
            ...
            if len(obbs)>0:
                ...
                pc_util.write_oriented_bbox(obbs[objectness_prob>DUMP_CONF_THRESH,:], os.path.join(dump_dir, '%s_pred_confident_bbox.ply'%(scan_names[i])))
                pc_util.write_oriented_bbox(obbs[np.logical_and(objectness_prob>DUMP_CONF_THRESH, pred_mask[i,:]==1),:], os.path.join(dump_dir, '%s_pred_confident_nms_bbox.ply'%(scan_names[i])))
                pc_util.write_oriented_bbox(obbs[pred_mask[i,:]==1,:], os.path.join(dump_dir, '%s_pred_nms_bbox.ply'%(scan_names[i])))
    ...
    gt_sem_cls = end_points['sem_cls_label'].cpu().numpy()
    ...
    for i in range(batch_size):
        ...
        if len(obbs)>0:
            ...
            pc_util.write_oriented_bbox(obbs, os.path.join(dump_dir, '%s_gt_bbox.ply'%(scan_names[i])))
    ...
    #         #     fout.write('\n')
    #         for j in range(gt_center.shape[1]):
    #             fout.write(np.str(gt_sem_cls[ii,j])) 
    #             fout.write(np.str(gt_center[i,j,0:3]))
    #         fout.close()
